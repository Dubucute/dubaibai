...
register({
  ...
  description:
    "Search uploaded documents for relevant information, then answer questions using RAG (Retrieval Augmented Generation).",
  parameters: {
    type: "object",
    properties: {
      ...
      document_id: {
        type: "string",
        description: "Optional: search only a specific document by ID.",
      },
      use_llm: {
        type: "boolean",
        description: "Whether to use an LLM to synthesize the answer.",
        default: true,
      },
    },
    required: ["query"],
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a document analysis assistant. Answer the user's question based ONLY on the provided document context. If the context doesn't contain enough information, say so.",
        },
        {
          role: "user",
