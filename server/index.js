    ...
    res.json({
      ...
      tasks: Object.fromEntries(
        Object.entries(require('./models').TASK_ROUTES).map(([task, route]) => [
          task,
          { description: route.description, chain: route.chain.map(id => ({ id, name: getModelInfo(id)?.name || id })) },
        ])
      ),
      all: getAllModels(),
      api: health,
...
app.post('/api/tools/:name/execute', async (req, res) => {
  ...
  try {
    const result = await tool.execute(req.body.args || req.body, { apiKey: req.headers['x-api-key'] || CONFIG.apiKey });
...
app.post('/api/filesystem/write', (req, res) => {
  ...
  try {
    ...
    if (!tool) return res.status(500).json({ success: false, error: 'Filesystem tool not loaded.' });
    tool.execute({ filepath, content, overwrite }).then(result => {
      res.json(result);
    }).catch(e => {
      res.status(500).json({ success: false, error: e.message });
    });
...
app.post('/api/filesystem/create-dir', (req, res) => {
  ...
  try {
    ...
    if (!tool) return res.status(500).json({ success: false, error: 'Filesystem tool not loaded.' });
    tool.execute({ folderpath }).then(result => {
      res.json(result);
    }).catch(e => {
      res.status(500).json({ success: false, error: e.message });
    });
...
app.post('/api/filesystem/read', (req, res) => {
  ...
  try {
    ...
    if (!tool) return res.status(500).json({ success: false, error: 'Filesystem tool not loaded.' });
    tool.execute({ filepath }).then(result => {
      res.json(result);
    }).catch(e => {
      res.status(500).json({ success: false, error: e.message });
    });
...
app.post('/api/filesystem/list', (req, res) => {
  ...
  try {
    ...
    if (!tool) return res.status(500).json({ success: false, error: 'Filesystem tool not loaded.' });
    tool.execute({ dirpath, maxDepth }).then(result => {
      res.json(result);
    }).catch(e => {
      res.status(500).json({ success: false, error: e.message });
    });
