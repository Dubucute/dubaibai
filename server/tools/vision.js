...
register({
  ...
  parameters: {
    type: 'object',
    properties: {
      image: { type: 'string', description: 'Base64-encoded image data (with or without data: prefix).' },
      prompt: { type: 'string', description: 'Question or instruction about the image.', default: 'Describe this image in detail.' },
      model: {
        type: 'string',
        description: 'Vision model to use.',
        enum: ['meta/llama-3.2-90b-vision-instruct', 'meta/llama-3.2-11b-vision-instruct',
  ...
  async execute(args) {
    const { image, prompt = 'Describe this image in detail.', model = 'meta/llama-3.2-90b-vision-instruct', max_tokens = 1024 } = args;
