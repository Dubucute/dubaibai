...
register({
  name: 'check_safety',
  description: 'Analyze text for content safety violations, jailbreak attempts, or topic policy compliance.',
  parameters: {
    type: 'object',
    properties: {
      ...
      model: {
        ...
        enum: ['nvidia/llama-3.1-nemoguard-8b-content-safety',
  ...
  async execute(args) {
    const { text, mode = 'content_safety', model = 'nvidia/llama-3.1-nemoguard-8b-content-safety' } = args;
    ...
    if (model.includes('jailbreak-detect')) {
      ...
      const systemMsg = mode === 'topic_control'
