...
function getAllModels() {
  // Load ranked models from the cleaned JSON file (generated by benchmark.js)
  // Use the stable temp path if available (e.g., on Vercel read‑only FS), fall back to default location
  ...
  try {
    ...
    console.warn("⚠️ Could not load ranked_models_clean.json, falling back to hardcoded models");
    // Fallback to the original hardcoded MODELS object
    const all = {};
    for (const category of Object.values(MODELS)) {
  ...
  // Build the model registry object expected by the client
  const all = {};
  for (const item of rankedData.data) {
    ...
    all[item.id] = {
      ...
      group: item.group || "",
      // Preserve any additional fields the frontend might use (e.g., vision, audio)
      ...item,
    };
  }
  return all;
