...
register({
  ...
  parameters: {
    type: 'object',
    properties: {
      input: { type: 'string', description: 'Text to embed (one item) or multiple items separated by newlines.' },
      mode: { type: 'string', enum: ['embed', 'rerank'], default: 'embed' },
      query: { type: 'string', description: 'Query for reranking mode.' },
      model: {
        type: 'string',
        description: 'Embedding model.',
        enum: ['nvidia/nv-embedqa-e5-v5', 'nvidia/nv-embed-v1',
               'nvidia/llama-nemotron-embed-1b-v2', 'baai/bge-m3',
